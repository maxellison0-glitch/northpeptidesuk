...
function formatMeta(type) {
  return type === 'pen' ? 'Tips + wipes included' : 'You reconstitute';
}
...
${configuredVariantButtons(activeFormat.variants)}
          ...
          <div class="config-total-row">
            ...
            <em id="config-total-note">${escapeHtml(activeFormat.type === 'pen' ? 'Complete kit included' : 'Vial only')}</em>
          </div>
...
const PRODUCT_CSS = `
    * { box-sizing: border-box; }
    ...
    .product-hero { max-width: 1160px; margin: 0 auto; padding: 30px 40px 32px; display: grid; grid-template-columns: minmax(280px, 0.9fr) minmax(320px, 1.1fr); gap: 34px; align-items: start; }
    ...
    .buy-card { padding: 26px; }
    .back-link { display: inline-block; margin-bottom: 18px; color: #63758A; font-family: 'DM Mono', monospace; font-size: 0.68rem; letter-spacing: 0.08em; text-transform: uppercase; text-decoration: none; }
    ...
    h1 { margin: 0 0 14px; font-family: Syne, sans-serif; font-size: clamp(2rem, 5vw, 3.2rem); line-height: 1; color: #10233F; }
    ...
    .summary { margin: 0 0 16px; font-size: 1rem; }
    ...
    .config-pen-kit-head span { color: #177245; font-family: 'DM Mono', monospace; font-size: 0.57rem; font-weight: 800; letter-spacing: 0.1em; text-transform: uppercase; }
...
const PRODUCT_CONFIG_SCRIPT = `
  <script>
    ...
    function refreshConfiguredOrder() {
      ...
      if (note) note.textContent = format.type === 'pen' ? 'Complete kit included' : (configuredBacWater ? 'Vial + BAC water' : 'Vial only');
